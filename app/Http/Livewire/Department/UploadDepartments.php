...
use App\Jobs\UploadDepartmentJob;
...
class UploadDepartments extends Component
{
    ...
    public function confirmed()
    {
        ...
        try {
            $csvValidator = $this->open(storage_path('app/public/' . $name), $this->departmentRules());
        ...
        dispatch((new UploadDepartmentJob(
            $id
        )))->onQueue('uploads')->delay(1);

        $this->reset();
        $this->alert('success', 'Successfully uploaded ' . count($this->csvData) . ' department(s).');
    ...
    {
        return view('livewire.department.upload-departments');
    }
}
