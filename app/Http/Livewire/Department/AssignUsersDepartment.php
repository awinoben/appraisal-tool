...
use App\Jobs\BulkAssignProjectJob;
...
class AssignUsersDepartment extends Component
{
    ...
    public function confirmed()
    {
        ...
        try {
            $csvValidator = $this->open(storage_path('app/public/' . $name), $this->assignEmployeeRules());
        ...
        dispatch((new BulkAssignProjectJob(
            $id,
            $name
        )))->onQueue('uploads')->delay(1);

        $this->reset();
        $this->alert('success', 'Successfully assigned ' . count($this->csvData) . ' employee(s).');
    ...
    {
        return view('livewire.department.assign-users-department');
    }
}
