...
use App\Jobs\UploadBranchesJob;
...
class UploadBranches extends Component
{
    ...
    public function confirmed()
    {
        ...
        try {
            $csvValidator = $this->open(storage_path('app/public/' . $name), $this->branchRules());
        ...
        dispatch((new UploadBranchesJob(
            $id
        )))->onQueue('uploads')->delay(1);

        $this->reset();
        $this->alert('success', 'Successfully uploaded ' . count($this->csvData) . ' branche(s).');
    ...
    {
        return view('livewire.branch.upload-branches');
    }
}
