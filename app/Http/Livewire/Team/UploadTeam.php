...
use App\Jobs\TeamUserAssignJob;
...
class UploadTeam extends Component
{
    ...
    public function confirmed()
    {
        ...
        try {
            $csvValidator = $this->open(storage_path('app/public/' . $name), $this->teamRules());
        ...
        dispatch((new TeamUserAssignJob(
            $id
        )))->onQueue('uploads')->delay(1);

        $this->reset();
        $this->alert('success', 'Successfully uploaded ' . count($this->csvData) . ' team-user(s).');
    ...
    {
        return view('livewire.team.upload-team');
    }
}
